...
func makeField(n: Int, m: Int,t: Int, mines: [[Int]]) -> [[String]] {
...
while true {
    ...
    for _ in 1...100 {
        let x1 = Int.random(in: 1...x)
        let y1 = Int.random(in: 1...y)
        if !sarr.contains([x1,y1]) {
            sarr.append([x1,y1])
        }
    }
    
    let trueField = makeField(n: Int(fieldParameters[0])!, m: Int(fieldParameters[1])!, t: Int(fieldParameters[2])!, mines: sarr)
